...
public class SpidSpMetadataResourceProvider implements RealmResourceProvider {
    ...
    public static final String FPA_METADATA_EXTENSIONS_NS = "http://ivaservizi.agenziaentrate.gov.it/docs/xsd/fatture/v1.2";
    ...
    public Response get() {
        try {
            ...
            int attributeConsumingServiceIndex = firstSpidProvider.getConfig()
                    .getAttributeConsumingServiceIndex() != null
                            ? firstSpidProvider.getConfig().getAttributeConsumingServiceIndex()
                            : 1;
            ...
            String[] attributeConsumingServiceNames = attributeConsumingServiceName != null
                    ? attributeConsumingServiceName.split(",")
                    : null;
            ...
            String[] organizationDisplayNames = strOrganizationDisplayNames != null
                    ? strOrganizationDisplayNames.split(",")
                    : null;
            ...
            if (firstSpidProvider.getConfig().isSignSpMetadata()) {
                ...
                KeyPair keyPair = new KeyPair((PublicKey) activeKey.getPublicKey(),
                ...
                signatureHelper
                        .setDigestMethod(firstSpidProvider.getSignatureAlgorithm().getXmlSignatureDigestMethod());
    ...
    private static void customizeEntityDescriptor(EntityDescriptorType entityDescriptor,
        ...
        final SpidSPMetadataResourceHelper helper = new SpidSPMetadataResourceHelper();
        ...
        if (organizationNames != null && organizationNames.length > 0 ||
            ...
            if (!StringUtil.isNullOrEmpty(otherContactPersonCompany)
                    || !StringUtil.isNullOrEmpty(otherContactPersonEmail)
                    || !StringUtil.isNullOrEmpty(otherContactPersonPhone)) {
                ContactType otherContactPerson = new ContactType(ContactTypeType.OTHER);

                if (!StringUtil.isNullOrEmpty(otherContactPersonCompany))
                    otherContactPerson.setCompany(otherContactPersonCompany);
                if (!StringUtil.isNullOrEmpty(otherContactPersonEmail))
                    otherContactPerson.addEmailAddress(otherContactPersonEmail);
                if (!StringUtil.isNullOrEmpty(otherContactPersonPhone))
                    otherContactPerson.addTelephone(otherContactPersonPhone);
            ...
            if (!StringUtil.isNullOrEmpty(billingContactPersonCompany)
                    || !StringUtil.isNullOrEmpty(billingContactPersonEmail)
                    || !StringUtil.isNullOrEmpty(billingContactPersonPhone)) {
                ContactType billingContactPerson = new ContactType(ContactTypeType.BILLING);

                if (!StringUtil.isNullOrEmpty(billingContactPersonCompany))
                    billingContactPerson.setCompany(billingContactPersonCompany);
                if (!StringUtil.isNullOrEmpty(billingContactPersonEmail))
                    billingContactPerson.addEmailAddress(billingContactPersonEmail);
                if (!StringUtil.isNullOrEmpty(billingContactPersonPhone))
